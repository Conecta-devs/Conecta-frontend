...
<template>
  <aside
    ...
    <a class="brand-mark flex items-center gap-[11px] px-[13px] text-[#172238] no-underline max-[850px]:justify-center max-[850px]:px-0"
      href="/" aria-label="Conecta, início">
      <span
        class="brand-icon grid size-9 place-items-center rounded-xl bg-[#253b73] font-['Space_Grotesk',sans-serif] text-lg font-bold text-white">C</span>
      <span
        class="brand-name font-['Space_Grotesk',sans-serif] text-[21px] font-bold tracking-[-.7px] max-[850px]:hidden">conecta<span
          class="text-[#e8755f]">.</span></span>
    </a>
    ...
    <nav class="sidebar-navigation mt-[66px] max-[850px]:mt-14" aria-label="Navegação do dashboard">
      ...
      <a v-for="item in menuItems" :key="item.label"
        ...
        :class="item.active ? 'navigation-item-active bg-[#e3e9fa] text-[#253b73]' : ''" href="#">
    ...
    <div class="sidebar-invite mt-auto mb-6 rounded-[14px] bg-[#fff6f1] p-[17px] max-[850px]:hidden">
      <div class="invite-icon mb-[13px] grid size-[34px] place-items-center rounded-[10px] bg-[#ffe5dc] text-[#e8755f]">
        <font-awesome-icon :icon="faUsers" />
      </div>
      <strong class="block font-['Space_Grotesk',sans-serif] text-[13px] font-semibold">Faça novas
        conexões</strong>
      <p class="my-2 text-[11px] leading-[1.5] text-[#7a8494]">
        Encontre pessoas com os mesmos interesses que você.
      </p>
      <a class="flex items-center gap-1 text-[11px] font-bold text-[#d9624c] no-underline" href="#comunidades">Explorar
        comunidades <font-awesome-icon :icon="faArrowRight" /></a>
    </div>
